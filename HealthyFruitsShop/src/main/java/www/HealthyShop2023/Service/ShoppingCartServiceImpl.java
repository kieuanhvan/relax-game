...
public class ShoppingCartServiceImpl implements ShoppingCartService{
	...
	public void remove(Long id) {
		...
		if(account != null && productDTO !=null) {
			Product product  = new Product();			product.setId(id);
			cartdao.deleteByProductAndAccount(product, account);
			map.remove(id);
		}
	...
	public Collection<ProductDTO> getItems() {
		...
		return new ArrayList<ProductDTO>(map.values());
	}
	@Override
	public int size() {
		// TODO Auto-generated method stub
	...
	public void loadmycart(List<CartSession> data) {
		map.clear();
		data.forEach(item ->{
			map.put(item.getId(), convertCartSSToProductDTO(item));
		});
	}
	private ProductDTO convertCartSSToProductDTO(CartSession cartss) {
		ProductDTO prodto = new ProductDTO();
		prodto.setId(cartss.getId());
